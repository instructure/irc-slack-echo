...
var server = Hapi.server({
  host: config.slack.outgoingWebhookServer.domain,
  port: config.slack.outgoingWebhookServer.port,
});
...
server.route({
  ...
  handler: function(request, h) {
    if (request.payload.token == config.slack.outgoingWebhookToken) {
      ...
      return Boom.unauthorized('Bad Token');
    }
  }
});
...
server.route({
  ...
  method: 'GET',
  handler: function(request, h) {
    return "Oh, hi. What are _you_ doing here?";
  }
});
...
var handleSlackInput = function(payload) {
  if (ircClient) {
    ...
    if (slackBotMethods.hasOwnProperty(command)) {
