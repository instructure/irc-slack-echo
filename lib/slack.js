...
var sendToSlack = function(content, username) {
  var postContent = {
    channel: config.slack.echoChannel,
    username: username || config.slack.botName,
    text: content
  };
...
var sendEcho = function(message, nick) {
  if (config.slack.includeSenderInBotName) {
    sendToSlack(message, config.slack.botName + ' <' + nick + '>');
  } else {
    sendToSlack('[' + nick + '] ' + message)
  }
}
...
var slackBotMethods = {
  ...
  'say': function(context) {
    ...
    var composed = "[" + mapping.slackNameToIrcName(context.user_name) + "] " + messageText;
...
var handleSlackInput = function(payload) {
  if (ircClient) {
    ...
    if (_.has(slackBotMethods, command)) {
      ...
      payload.commandless_words.unshift(command)
      slackBotMethods['say'](payload);
    }
  }
};
